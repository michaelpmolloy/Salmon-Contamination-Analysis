...
## cluster analysis
```{r}
optimalClusterPlot <- fviz_nbclust(farmedSalmon[,c(-1,-11)], kmeans, method = "wss") + scale_y_continuous(labels = unit_format(unit = "K", scale = 1e-3)) + labs(x="Cluster No", y="Total WSS") + theme(plot.title = element_text(hjust = 0.5), axis.text = element_text(size = 7), axis.title=element_text(size=8)) 
